...
public class ItemSearchServiceImpl implements ItemSearchService {
	...
	public Map<String, Object> search(Map searchMap) {
		...
		if (!"".equals(category)) {//如果没有分类名称
			map.putAll(searchBrandAndSpenList(category)); 
		} else {
			if (categoryList.size()>0) {
				map.putAll(searchBrandAndSpenList(categoryList.get(0))); 
			}
		}
	...
	private Map searchList(Map searchMap) {
		...
		map.put("total", page.getTotalElements());//返回中记录数
		return map;
	 ...
	 * @return
	 */
	private Map searchBrandAndSpenList(String category) {
